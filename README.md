...
面向体育、人机交互等（最初是动漫/插画数据集）的多标签智能图像分类标注系统，动机是分层、超长的图像细粒度文本描述的人工成本太高，耗时较多，而利用成熟的多模态模型可以完成自动化高质量的图像描述来完成标注，进而为下游任务提供高质量的文本描述数据集，为姿态识别、文生图任务的模型训练等提供支持。支持 **本地 VLM 模型自动标注** + **远程 API 自动标注** + **人工校正** 三种模式。
