...
面向文生图模型合成数据、体育、人机交互等（最初是二次元动漫/插画数据集，老二刺猿了）的多标签智能图像分类标注系统，开发的动机是获得分层、超长的图像细粒度文本描述的人工成本太高，耗时较多，而利用成熟的多模态模型可以完成自动化高质量的图像描述来完成标注，进而为下游任务提供高质量的文本描述数据集，为姿态识别、文生图任务的模型训练等提供支持。支持 **本地 VLM 模型自动标注** + **远程 API 自动标注** + **人工校正** 三种模式。（还要非常感谢智谱团队的 GLM 5.1 模型出色的 agent 能力，项目大部分编码由其完成，Vibe coding 开源心目第一强）。关注我的 X 账号 https://x.com/flyingpetal472

## 功能特性

- 10 大分类、60+ 预设标签（性别、发色、发型、瞳色、角色特征、服装、姿势、背景、风格、数量）
- 本地多模态 VLM 模型自动标注（部署 Qwen3.5-4B 等，完全离线）
- 远程 API 自动标注（OpenAI / Anthropic）
- Web 可视化标注界面，支持键盘快捷键
